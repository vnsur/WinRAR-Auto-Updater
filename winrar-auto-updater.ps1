...
.FUNCTIONALITY
    - Auto-detect current WinRAR version
    - Check latest version from RARLab.com
...
Write-Host "  🚀 $($ScriptInfo.Name) v$($ScriptInfo.Version)" -ForegroundColor Yellow
...
if ($register) {
    if (!([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator")) { 
        Write-Host "⚠️  Administrator privileges required for scheduling tasks!" -ForegroundColor Yellow
        Start-Process powershell.exe "-NoProfile -ExecutionPolicy Bypass -File `"$scriptPath`" -register" -Verb RunAs
        exit 
    }
} else {
    if (!([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator")) { 
        Write-Host "⚠️  Administrator privileges required for WinRAR installation!" -ForegroundColor Yellow
...
if ($register) {
    try {
        ...
        Write-Host "❌ Failed to register scheduled task 'AutoUpdateWinRAR_DarkMethod'..." -ForegroundColor Red
...
$webClient.Headers.Add("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) WinRAR-AutoUpdater/$($ScriptInfo.Version)")
...
function Install-WinRAR($versionInfo) {
    try {
        ...
        Write-Host "⬇️  Downloading WinRAR $($versionInfo.DisplayName)..." -ForegroundColor Yellow
...
if (-not (Get-Process -Name "WinRAR" -ErrorAction SilentlyContinue)) {
    ...
    try {
        if($currentVersion) {
            ...
            Log "⚠️  WARNING: WinRAR is not installed. Skipping update."
            Write-Host "⚠️  WinRAR not found on this system" -ForegroundColor Yellow
        }
    } catch {
    ...
    Log "⚠️  WinRAR is currently running. Skipping update to avoid conflicts."
    Write-Host "⚠️  WinRAR is running - update skipped" -ForegroundColor Yellow
}
